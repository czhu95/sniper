...
uint64_t start[64], end[64], length[64];
   ...
   while (SimGMMCorePull(msg) != -1)
   {
      ...
      switch (msg.type)
      {
         case POLICY_INIT:
            ...
            start[seg] = msg.payload[0];
            end[seg] = msg.payload[1];
            length[seg] = end[seg] - start[seg];
            block_map[seg] = new bool[length[seg] / block_size + 1];
            for (int i = 0; i < length[seg] / block_size + 1; i ++)
               block_map[seg][i] = false;
            break;
         case EX_REQ:
         case SH_REQ:
            seg = msg.policy - 8;
            ...
            addr = msg.payload[0];
            // printf("addr: %lx\n", addr);
            block_num = (addr >> block_logsize) - (start[seg] >> block_logsize);
            // printf("blocknum: %d\n", block_num);
            if (!block_map[seg][block_num])
            {
               block_map[seg][block_num] = true;
               msg.type = TLB_INSERT;
               msg.component = GMM_CORE;
               msg.receiver = node_id;
               msg.payload[0] = MAX(addr & block_mask, start[seg]);
               msg.payload[1] = MIN((addr & block_mask) + block_size, end[seg]);
