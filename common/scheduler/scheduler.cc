...
core_id_t Scheduler::findFirstFreeCore()
{
   ...
   for (core_id_t core_id = (core_id_t)Sim()->getConfig()->getApplicationCores(); core_id < (core_id_t)Sim()->getConfig()->getTotalCores(); core_id++)
   {
      if (Sim()->getGMMCoreManager()->getCoreFromID(core_id)->getState() == Core::IDLE)
      {
         return core_id;
      }
   }
...
void Scheduler::printMapping()
{
   ...
   for (core_id_t core_id = (core_id_t)Sim()->getConfig()->getApplicationCores(); core_id < (core_id_t)Sim()->getConfig()->getTotalCores(); core_id++)
   {
      ...
      {
         char state;
         switch(Sim()->getThreadManager()->getThreadState(Sim()->getGMMCoreManager()->getCoreFromID(core_id)->getThread()->getId() ))
         {
            case Core::INITIALIZING:
               state = 'I';
               break;
         ...
         std::cout << "( t" << Sim()->getGMMCoreManager()->getCoreFromID(core_id)->getThread()->getId() << "@c" << core_id << "[" << state << "])" ;
      }
      else
      {
         std::cout << "( tx" << "@c" << core_id  << "[i])";
