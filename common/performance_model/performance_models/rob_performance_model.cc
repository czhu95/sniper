...
RobPerformanceModel::RobPerformanceModel(Core *core)
    ...
    , rob_timer(core,
       ...
       core->getId() < (core_id_t)Sim()->getConfig()->getApplicationCores() ?
             Sim()->getCfg()->getIntArray("perf_model/core/interval_timer/dispatch_width", core->getId()) :
             Sim()->getCfg()->getIntArray("perf_model/gmm/core/interval_timer/dispatch_width", core->getId()),
       core->getId() < (core_id_t)Sim()->getConfig()->getApplicationCores() ?
             Sim()->getCfg()->getIntArray("perf_model/core/interval_timer/window_size", core->getId()) :
             Sim()->getCfg()->getIntArray("perf_model/gmm/core/interval_timer/window_size", core->getId())
    )
{
}
