...
UserTraceManager::UserTraceManager()
...
String UserTraceManager::getFifoName(app_id_t app_id, UInt64 thread_num, bool response, bool create)
{
...
thread_id_t UserTraceManager::createThread(app_id_t app_id, SubsecondTime time, thread_id_t creator_thread_id)
{
...
thread_id_t UserTraceManager::newThread(app_id_t app_id, bool first, bool init_fifo, bool spawn, SubsecondTime time, thread_id_t creator_thread_id)
{
...
app_id_t UserTraceManager::createApplication(SubsecondTime time, thread_id_t creator_thread_id)
{
   ScopedLock sl(m_lock);
...
void UserTraceManager::signalDone(TraceThread *thread, SubsecondTime time, bool aborted)
{
   ScopedLock sl(m_lock);
...
void UserTraceManager::endApplication(TraceThread *thread, SubsecondTime time)
{
   for(std::vector<TraceThread *>::iterator it = m_threads.begin(); it != m_threads.end(); ++it)
   {
...
UInt64 UserTraceManager::getProgressValue()
{
   UInt64 value = 0;
   for(std::vector<TraceThread *>::iterator it = m_threads.begin(); it != m_threads.end(); ++it)
   {
...
void SystemTraceManager::cleanup()
{
   for(std::vector<TraceThread *>::iterator it = m_threads.begin(); it != m_threads.end(); ++it)
      delete *it;
...
void SystemTraceManager::signalDone(TraceThread *thread, SubsecondTime time, bool aborted)
{
   if (thread->m_stopped)
   {
      return;
