...
ReplicationPolicy::processNextReqFromL2Cache(IntPtr address)
{
   ...
   if (! m_dram_directory_req_queue_list->empty(address))
   {
      ...
      if (shmem_req->getShmemMsg()->getMsgType() == PrL1PrL2DramDirectoryMSI::ShmemMsg::SH_REQ)
      {
...
ReplicationPolicy::processShReqFromL2Cache(ShmemReq* shmem_req, Byte* cached_data_buf)
{
   ...
   retrieveDataAndSendToL2Cache(PrL1PrL2DramDirectoryMSI::ShmemMsg::SH_REP, requester, address, NULL, shmem_req->getShmemMsg());

   MYLOG("End @ %lx", address);
}

void
ReplicationPolicy::retrieveDataAndSendToL2Cache(PrL1PrL2DramDirectoryMSI::ShmemMsg::msg_t reply_msg_type,
      core_id_t receiver, IntPtr address, Byte* cached_data_buf, ShmemMsg *orig_shmem_msg)
{
   ...
   core_id_t dram_node = m_replicas.count(repl_addr) ? m_core_id :
                           m_dram_controller_home_lookup->getHome(phys_address);
