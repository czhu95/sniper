...
ADDRINT handleMagic(THREADID threadid, const CONTEXT * ctxt, ADDRINT gax, ADDRINT gbx, ADDRINT gcx)
{
   ...
   if (KnobUseResponseFiles.Value() && thread_data[threadid].running && thread_data[threadid].output)
   {
      if (gax == SIM_CMD_GMM_CORE_MESSAGE)
      {
         ...
         fprintf(stderr, "[%d]GMMCoreMessage: seg=%d, id=%d, type=%d, receiver=%d, payload[0]=%lx, payload[1]=%lx\n", app_id, gmm_core_msg.segid, gmm_core_msg.policy, gmm_core_msg.type, gmm_core_msg.receiver, gmm_core_msg.payload[0], gmm_core_msg.payload[1]);
      ...
      else if (gax == SIM_CMD_GMM_CORE_PULL)
      {
         ...
         fprintf(stderr, "[%d]GMMCorePull: seg=%d, id=%d, type=%d, addr1=%lx, addr2=%lx\n", app_id, gmm_core_msg.segid, gmm_core_msg.policy, gmm_core_msg.type, gmm_core_msg.payload[0], gmm_core_msg.payload[1]);
